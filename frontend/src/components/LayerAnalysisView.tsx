...
import { Eye, Layers, Compass, LayoutGrid, Check, Settings, Info, ArrowUpRight, Lightbulb, Users, X } from 'lucide-react';
    ...
    <div className="flex h-[calc(100vh-6.5rem)] flex-col bg-slate-50 dark:bg-slate-950" id="view-2-layer-analysis">
      ...
      <div className="border-b border-slate-200 bg-white px-4 py-3 transition-colors duration-300 dark:border-slate-800 dark:bg-slate-950">
        <div className="mx-auto max-w-7xl flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          ...
          {/* 5 Tabs Control Box */}
          <div id="operational-filters-tabs" className="flex flex-nowrap items-center gap-1.5 overflow-x-auto w-full pb-1">
            {ANALYSIS_TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`relative rounded-full px-4 py-2 text-[11px] font-semibold tracking-tight transition-all duration-300 flex items-center space-x-1.5 whitespace-nowrap shrink-0 ${
                    isSelected
      ...
      <div className="flex-1 relative bg-slate-50/50 dark:bg-slate-950/20 p-2 sm:p-3 md:p-4 lg:p-6">
        <div className="h-full w-full relative">
          ...
          {/* Mobile floating buttons to toggle layers and legend */}
          <div className="absolute top-4 right-4 z-20 flex flex-col space-y-2 lg:hidden">
            <button
              onClick={() => setMobileLayersOpen(true)}
              className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-slate-950/85 text-white shadow-xl hover:bg-slate-900 transition-all pointer-events-auto backdrop-blur-md active:scale-95"
              title="Przełącz warstwy"
            >
          ...
          {/* Desktop Floating Panel showing Layer Options */}
          <div className="hidden lg:block absolute top-4 right-4 z-20 w-72 max-w-xs rounded-2xl border border-white/10 bg-slate-950/85 p-4 shadow-2xl backdrop-blur-xl animate-slide-in-right">
          ...
          {/* Desktop Elegant Floating Legend: Glassmorphism Style in Bottom Corner */}
          <div className="hidden lg:block absolute bottom-4 right-4 z-20 w-64 rounded-2xl border border-white/10 bg-slate-950/85 p-4 shadow-2xl backdrop-blur-xl transition-all animate-fade-in">
          ...
          {/* Mobile Layer Options Modal */}
          {mobileLayersOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-sm p-4 lg:hidden">
              <div className="relative w-full max-w-xs rounded-2xl border border-white/10 bg-slate-950 p-5 shadow-2xl animate-fade-in-up">
                <button
                ...
                <div className="space-y-2">
                  {layerOptions.map((layer) => {
                    ...
                    return (
                      <button
                        key={layer.id}
                        id={`btn-layer-mobile-${layer.id}`}
                        onClick={() => { handleLayerToggle(layer.id); setMobileLayersOpen(false); }}
                        className={`flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-xs font-semibold border transition-all duration-200 ${
                          isActive
                            ? 'bg-white border-white text-slate-950 shadow-md'
                            : 'bg-slate-900/60 border-slate-800/80 text-slate-400 hover:bg-slate-800 hover:border-emerald-500/30 hover:text-slate-200'
          ...
          {mobileLegendOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-sm p-4 lg:hidden">
              <div className="relative w-full max-w-xs rounded-2xl border border-white/10 bg-slate-950 p-5 shadow-2xl animate-fade-in-up max-h-[90vh] overflow-y-auto">
                <button
